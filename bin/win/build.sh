...
${ACME}/acme.exe -f cbm -l build/labels -r build/lores.lst -o build/lores.ml.prg code/lores.asm 2> build/build.err
result=$?
cat build/build.err
[ ${result} -eq 0 ] || exit 1
rm build/build.err
${ACME}/acme.exe -f cbm -l build/labels -r build/rleplayer.lst -o build/rleplayer.ml.prg code/rleplayer.asm 2> build/build.err
result=$?
cat build/build.err
[ ${result} -eq 0 ] || exit 1
rm build/build.err
